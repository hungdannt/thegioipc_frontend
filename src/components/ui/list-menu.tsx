...
const ListMenu = ({ dept, data, hasSubMenu, menuIndex }: any) => {
  ...
  return (
    <li className="relative">
      <Link
        href={data.path}
        className="flex items-center justify-between py-2 ps-5 font-semibold xl:ps-7 pe-3 xl:pe-3.5 hover:text-red hover:bg-gray-300"
      >
        {t(data.label)}
        {(data.subMenu || data.columns) && (
      ...
      {data.columns && <MegaMenu columns={data.columns} />}
    </li>
  );
};
